...
flags.DEFINE_string("output_line_prefix_tag", "", "Prepended to output lines.")
...
def main(_):
  ...
  if FLAGS.decode_to_file and FLAGS.output_line_prefix_tag:
    ...
    decode_filename_prefixed = "%s-%s" % (
        decode_filename_original, FLAGS.output_line_prefix_tag)
    tf.logging.info(
        "Writing prefexed decodes into %s" % decode_filename_prefixed)
    # Read original lines.
