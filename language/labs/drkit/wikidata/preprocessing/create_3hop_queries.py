  ...
  for entity in tqdm(obj_to_index):
    ...
    for fact_1_idx in obj_to_index[entity]:
      ...
      for fact_2_idx in subj_to_index[entity]:
        ...
        tail = fact_2["object"]["wikidata_id"]
        if len(obj_to_index[tail]) > MAX_IN_DEGREE:
          continue
        if tail not in subj_to_index:
          continue
        for fact_3_idx in subj_to_index[tail]:
          fact_3 = queries[fact_3_idx]
          if (fact_3["object"]["wikidata_id"] == fact_2["subject"]
              ["wikidata_id"]):
            continue
          if (fact_3["object"]["wikidata_id"] == fact_1["subject"]
              ["wikidata_id"]):
            continue
          mh_qry = {}
          mh_qry["subject"] = fact_1["subject"]
          mh_qry["object"] = fact_3["object"]
          mh_qry["relation"] = [
              fact_1["relation"], fact_2["relation"], fact_3["relation"]
          ]
          mh_qry["para_id"] = [
              fact_1["para_id"], fact_2["para_id"], fact_3["para_id"]
          ]
          mh_qry["bridge_0"] = {}
          mh_qry["bridge_0"]["wikidata_id"] = fact_1["object"]["wikidata_id"]
          mh_qry["bridge_0"]["name"] = fact_1["object"]["name"]
          mh_qry["bridge_0"]["aliases"] = fact_1["object"]["aliases"]
          mh_qry["bridge_0"]["mention_1"] = fact_1["object"]["mention"]
          mh_qry["bridge_0"]["mention_2"] = fact_2["subject"]["mentions"]
          mh_qry["bridge_1"] = {}
          mh_qry["bridge_1"]["wikidata_id"] = fact_2["object"]["wikidata_id"]
          mh_qry["bridge_1"]["name"] = fact_2["object"]["name"]
          mh_qry["bridge_1"]["aliases"] = fact_2["object"]["aliases"]
          mh_qry["bridge_1"]["mention_1"] = fact_2["object"]["mention"]
          mh_qry["bridge_1"]["mention_2"] = fact_3["subject"]["mentions"]
          mh_qry["id"] = fact_1["id"] + "_" + fact_2["id"] + "_" + fact_3["id"]
          multihop_queries.append(mh_qry)
          paragraph_ids.add(fact_1["para_id"])
          paragraph_ids.add(fact_2["para_id"])
          paragraph_ids.add(fact_3["para_id"])
          entity_ids.add(fact_1["subject"]["wikidata_id"])
          entity_ids.add(fact_2["subject"]["wikidata_id"])
          entity_ids.add(fact_3["subject"]["wikidata_id"])
