...
"""BERT finetuning runner with model distillation for MNLI and SST2."""
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      if set_type == "test":
        ...
        # The last three indices correspond to probabilities of contradiction,
        # entailment and neutral.
        label = [float(line[-3]), float(line[-2]), float(line[-1])]
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if isinstance(example, PaddingInputExample):
    return InputFeatures(
        ...
        label_id=[0.0 if i != 0 else 1.0 for i, _ in enumerate(label_list)],
  ...
  feature = InputFeatures(
      ...
      # example.label is now a vector of probabilities
...
def file_based_convert_examples_to_features(examples, label_list,
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    # Creating float list for labels id since we are using distillation
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf_estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        ...
        label_argmax = tf.argmax(label_ids, axis=-1, output_type=tf.int32)
