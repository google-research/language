...
# STEP 2
# Build the WIKI set of queries to extract the model
...
# STEP 4.1
# Combine the queries with the outputs of the victim model, but watermark a fraction of them
...
python -m language.bert_extraction.steal_bert_classifier.utils.preprocess_distill_input_watermark \
  --task_name=$TASK_NAME \
  ...
  --watermark_path=$EXTRACTION_DATA/watermark.tsv
...
# In addition, check the predictions of the extracted model on the watermarked inputs
...
python -m language.bert_extraction.steal_bert_classifier.models.run_classifier_distillation \
  ...
  --do_eval=true \
  --do_predict=true \
  --predict_input_file=$EXTRACTION_DATA/watermark.tsv \
  --predict_output_file=$EXTRACTION_DATA/watermark_output.tsv \
  --do_lower_case=true \
...
# STEP 6
# Verify the extent to which the watermark was predicted correctly
python -m language.bert_extraction.steal_bert_classifier.utils.verify_watermark \
  --watermark_path=$EXTRACTION_DATA/watermark.tsv \
  --watermark_probs=$EXTRACTION_DATA/watermark_output.tsv \
  --task_name=$TASK_NAME
