...
flags.DEFINE_string("predict_output_file", None,
                    "Output file to dump outputs from a QA model.")
...
class BoolQExample(object):
  ...
  def __init__(self, guid, text_a, text_b=None, label=None, probs=None):
    ...
    Args:
      ...
      probs: Soft probability vector between labels.
    """
    self.guid = guid
    self.text_a = text_a
    self.text_b = text_b
...
class InputFeatures(object):
  ...
  def __init__(self, input_ids, input_mask, segment_ids, label_id, probs):
...
def _create_examples(filename, set_type):
  ...
  with tf.gfile.Open(filename) as f:
    for i, line in enumerate(f):
      ...
      if "soft_answer" in data:
        probs = data["soft_answer"]
      elif label == "Yes":
        probs = [1.0, 0.0]
      else:
        probs = [0.0, 1.0]
...
def get_custom(filename):
  return _create_examples(filename, "custom")
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if ex_index < 5:
    ...
    tf.logging.info("probabilities: %s", example.probs)
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, per_example_loss, logits,
     probabilities) = create_model(bert_config, is_training, input_ids,
                                   input_mask, segment_ids, gt_probs,
...
def main(_):
  ...
  if (not FLAGS.do_train and not FLAGS.do_eval_dev and
      not FLAGS.do_eval_test and not FLAGS.do_predict):
    raise ValueError(
  ...
  if FLAGS.do_predict:
    ...
    for result in estimator.predict(
      ...
      output_predictions.append(
          json.dumps({
              "passage": predict_examples[counter].text_a,
              "question": predict_examples[counter].text_b,
              "title": "Steal Dataset",
              "answer": answer,
              "soft_answer": [float(probs_result[0]),
                              float(probs_result[1])]
          }))
      counter += 1

    tf.logging.info("%d in original file, %d predicted", len(predict_examples),
                    counter)
