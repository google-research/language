...
This is copied directly from the original BERT reference implementation and
is minimally modified to:
  1. Work on modern TensorFlow releases (by updating LayerNorm).
  2. Added option to use different sequences for queries and key-values in
     `transformer_model`.
"""
...
from tf_slim import layers as slim_layers
...
def transformer_model(input_tensor,
  ...
  Args:
    ...
    input_kv_tensor: float Tensor of shape [batch_size, seq_length_kv,
      seq_dim_kv]. If specified, this will be used for the initial layer of keys
      and values for self-attention. `input_tensor` will still be used for
      queries and resnet connections.
      ...
      with tf.variable_scope("attention"):
        with tf.variable_scope("self"):
          attention_output = attention_layer(
              ...
              to_tensor=(input_kv_tensor
                         if input_kv_tensor is not None else layer_input),
              attention_mask=attention_mask,
