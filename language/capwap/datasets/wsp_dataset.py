...
"""Creates a tf.data.Dataset for a weakly supervised generation dataset.

This dataset type is used for WSP generation using the text planner
(GENERATE in Alg. 1).
"""
...
def parse_example(serialized_example):
  ...
  features = tf.io.parse_single_example(
      ...
      features=dict(
          ...
          question_ids=tf.io.FixedLenFeature(shape=[], dtype=tf.string),
          questions=tf.io.FixedLenFeature(shape=[], dtype=tf.string),
          answers=tf.io.FixedLenFeature(shape=[], dtype=tf.string),
          captions=tf.io.FixedLenFeature(shape=[], dtype=tf.string),
          object_positions=tf.io.FixedLenFeature(shape=[], dtype=tf.string),
          object_features=tf.io.FixedLenFeature(shape=[], dtype=tf.string)))
  return features
