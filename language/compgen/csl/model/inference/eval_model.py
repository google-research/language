...
flags.DEFINE_string("output", "predictions.txt", "Output txt file.")
...
def run_inference(writer, wrapper, examples, fallback_predictions, step=None):
  """Run inference."""
  counts, predictions = eval_utils.eval_model(
      wrapper, examples, fallback_predictions, verbose=FLAGS.verbose)
  ...
  if FLAGS.write:
    ...
    txt_utils.write_txt(predictions, filename)
...
def main(unused_argv):
  ...
  if FLAGS.poll:
    ...
    checkpoint, step = inference_utils.get_checkpoint(wrapper, FLAGS.model_dir,
                                                      FLAGS.checkpoint)
    run_inference(writer, wrapper, examples, fallback_predictions, step=step)
