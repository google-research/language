...
class TargetGrammarTest(tf.test.TestCase):
  ...
  def test_can_parse_with_nt_true(self):
    rules = [
        target_grammar.TargetCfgRule.from_string("ROOT => foo ##C"),
        target_grammar.TargetCfgRule.from_string("C => bar ##A")
    ]
    self.assertTrue(
        target_grammar.can_parse("foo bar ##A", rules, verbose=True))

  def test_can_parse_with_nt_false(self):
    rules = [
        target_grammar.TargetCfgRule.from_string("ROOT => foo ##C"),
        target_grammar.TargetCfgRule.from_string("C => bar")
    ]
    self.assertFalse(target_grammar.can_parse("buzz ##C", rules, verbose=True))
