...
def _has_leading_or_trailing_punctuation(source):
  # TODO(petershaw): Let this character set be configured per dataset.
  if source[0] in {".", "'"}:
    return True
  if source[-1] in {".", "'"}:
    return True
  ...
  for sequence_len in range(max(target_len, source_len), 0, -1):
    for source_start_idx in range(0, source_len - sequence_len + 1):
      ...
      for target_start_idx in range(0, target_len - sequence_len + 1):
        ...
        if _has_leading_or_trailing_punctuation(source_span):
          continue
