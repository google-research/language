  ...
  if dataset_name == 'spider':
    ...
    for query in data:
      for example in query['sentences']:
        ...
        for variable, value in sorted(example['variables'].items(), key = lambda x: len(x[0]), reverse=True):
          if not value:
            value = '%'
        ...
        if 'advising' in dataset_name.lower():
          ...
          if 'count' in anon_sql.lower():
            ...
            count_end_idx = count_start_idx + anon_sql.lower()[count_start_idx:].index(')')
            ...
            if ',' in anon_sql[count_start_idx:count_end_idx]:
              ...
              anon_sql = anon_sql[:count_start_idx] + problem_segment + anon_sql[count_end_idx:]
          prev_token = ''
          bad_tokens = set()
          for token in anon_sql.split():
            if prev_token == '=':
              if token[0] in {'"', '\''} and token[-1] in {'"', '\''} and token[-2].isnumeric() and not token[1].isnumeric():
        ...
        # Two specific exceptions on utterances that need correction or take a long time to process.
        if nl == 'What is the number of businesses user Michelle reviews per month ?':
          anon_sql = 'select count(distinct(review.text)), review.month from review where review.user_id in (select user_id from user where user.name = \'Michelle\') group by review.month;'

        if nl == 'return me the number of papers in " University of Michigan " in Databases area .':
          results = '121572'
          ...
          if 'advising' not in dataset_name and nl in cache and cache[nl] != anon_sql:
            print(nl)
            print(anon_sql)
            print(cache[nl])
            keep_going = input('Allow this to happen? This utterance will be mapped to the second query.').lower() == 'y'
            if not keep_going:
...
if __name__ == '__main__':
  ...
  cache_path = os.path.join(dataset_name, dataset_name + '_cache.json')
  errors_filepath = os.path.join(dataset_name, 'exec_errors.txt')

  main(dataset_name, cache_path, errors_filepath, splits)
