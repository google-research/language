...
class RaceProcessor:
  ...
  def get_train_examples(self, data_dir):
    ...
    return self.read_examples(os.path.join(data_dir, "RACE", "train"))

  def get_dev_examples(self, data_dir):
    """Gets a collection of `InputExample`s for the dev set."""
    return self.read_examples(os.path.join(data_dir, "RACE", "dev"))

  def get_test_examples(self, data_dir):
    """Gets a collection of `InputExample`s for prediction."""
    return self.read_examples(os.path.join(data_dir, "RACE", "test"))
  ...
  def read_examples(self, data_dir):
    ...
    for level in ["middle", "high"]:
      if level == "middle" and self.high_only:
        continue
      if level == "high" and self.middle_only:
        continue
  ...
  if isinstance(example, PaddingInputExample):
    ...
    for ending in example.endings:
      ...
      if len(context_tokens_choice) + len(ending_tokens) > max_seq_length - 3:
        context_tokens_choice = context_tokens_choice[:max_seq_length - 3 -
                                                      len(ending_tokens)]
      tokens = ["[CLS]"] + context_tokens_choice + ["[SEP]"] + ending_tokens + [
          "[SEP]"
      ]
    ...
    if example_index < 5:
      ...
      for choice_idx, (tokens, input_ids, input_mask, segment_ids) in \
        ...
        tf.logging.info("input_ids: {}".format(" ".join(map(str, input_ids))))
        tf.logging.info("input_mask: {}".format(" ".join(map(str, input_mask))))
        tf.logging.info("segment_ids: {}".format(" ".join(
            map(str, segment_ids))))
